...
const CARD_FIELD_RULES = [
  ...
  `每条 bullet 务必严格控制在 ${VALIDATION_RULES.bullet.bulletMin} 到 ${VALIDATION_RULES.bullet.bulletMax} 个字符之间，拒绝长篇大论，用加粗提取核心。`,
...
const CARD_CONTENT_RULES = [
  '所有用户可见内容必须使用简体中文。',
  '不要输出 HTML、emoji、代码块。但强烈建议在长段落中使用 **加粗** 来高亮核心得分点。',
  ...
  'bullet-1 只负责“标准面试回答”，读者按顺序读完 bullets 后，应能拼出一段完整的回答。',
  'bullet-1 可以吸收“回答里的关键论据”补强，但不能退化成关键词堆砌。',
  'bullet-1 务必详实、有血有肉，必须有足够展开度，且分点列出 3-8 条。',
  'bullet-1 必须写成前端面试生真正会说出口的完整回答，不要出现“定义：”“关键论据：”“总结：”“第一点：”这类提纲标签。',
  'bullet-1 的每条 bullet 都应该像连续口语句子，读起来能自然衔接。',
  'bullet-2 只负责“面试官最可能追问的问题”，每条必须是一整段包含“追问问题？——深入实战的答题方向和原理解释”。',
  'bullet-3 只负责“这道题的易错点”，每条必须写透“错在哪里 / 真实工程场景容易怎么踩坑 / 怎么避坑”。',
