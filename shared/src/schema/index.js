...
export const CardDocumentSchema = z.object({
    ...
    cards: z.array(CardSchema).refine((cards) => cards.length >= 4 && cards.length <= 8, { message: '卡片总数必须在 4~8 之间' }).refine((cards) => cards[0]?.type === 'cover', { message: '第一页必须是 cover' }).refine((cards) => cards[cards.length - 1]?.type === 'summary', { message: '最后一页必须是 summary' }).refine((cards) => cards.slice(1, -1).every((c) => c.type === 'bullet'), { message: '中间页只能是 bullet' }),
});
